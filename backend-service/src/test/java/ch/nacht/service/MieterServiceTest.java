...
public class MieterServiceTest {
    ...
    // ==================== Ladepunkt Tests (Spec Ladestromtarif.md) ====================

    @Test
    void saveMieter_NoLadepunkt_SkipsUniquenessCheck() {
        Mieter newMieter = new Mieter("Ohne Ladepunkt", LocalDate.of(2025, 1, 1), 5L);
        newMieter.setStrasse("Strasse");
        newMieter.setPlz("3000");
        newMieter.setOrt("Bern");
        newMieter.setMietende(LocalDate.of(2025, 12, 31));
        ...
        when(organizationContextService.getCurrentOrgId()).thenReturn(testOrgId);
        when(mieterRepository.existsOverlappingMieterBounded(eq(5L), any(), any(), eq(-1L)))
                .thenReturn(false);
        when(mieterRepository.save(any(Mieter.class))).thenAnswer(invocation -> invocation.getArgument(0));

        Mieter result = mieterService.saveMieter(newMieter);

        assertNull(result.getLadepunkt());
        verify(mieterRepository, never()).existsByLadepunkt(any(), anyLong());
    }

    @Test
    void saveMieter_UniqueLadepunkt_SavesSuccessfully() {
        Mieter newMieter = new Mieter("Mit Ladepunkt", LocalDate.of(2025, 1, 1), 6L);
        newMieter.setStrasse("Strasse");
        newMieter.setPlz("3000");
        newMieter.setOrt("Bern");
        newMieter.setMietende(LocalDate.of(2025, 12, 31));
        newMieter.setLadepunkt("LP-01");

        when(organizationContextService.getCurrentOrgId()).thenReturn(testOrgId);
        when(mieterRepository.existsOverlappingMieterBounded(eq(6L), any(), any(), eq(-1L)))
                .thenReturn(false);
        when(mieterRepository.existsByLadepunkt("LP-01", -1L)).thenReturn(false);
        when(mieterRepository.save(any(Mieter.class))).thenAnswer(invocation -> invocation.getArgument(0));

        Mieter result = mieterService.saveMieter(newMieter);

        assertEquals("LP-01", result.getLadepunkt());
        verify(mieterRepository).existsByLadepunkt("LP-01", -1L);
    }

    @Test
    void saveMieter_LadepunktAlreadyUsedByOtherMieter_ThrowsException() {
        Mieter newMieter = new Mieter("Doppelter Ladepunkt", LocalDate.of(2025, 1, 1), 7L);
        newMieter.setStrasse("Strasse");
        newMieter.setPlz("3000");
        newMieter.setOrt("Bern");
        newMieter.setMietende(LocalDate.of(2025, 12, 31));
        ...
        when(mieterRepository.existsOverlappingMieterBounded(eq(7L), any(), any(), eq(-1L)))
                .thenReturn(false);
        when(mieterRepository.existsByLadepunkt("LP-01", -1L)).thenReturn(true);
        ...
        assertThat(exception.getMessage(), containsString("Ladepunkt ist bereits einem anderen Mieter zugeordnet"));
        verify(mieterRepository, never()).save(any());
    }

    @Test
    void saveMieter_BlankLadepunkt_NormalisedToNull() {
        // Leereingaben duerfen nicht miteinander kollidieren
        Mieter newMieter = new Mieter("Leerer Ladepunkt", LocalDate.of(2025, 1, 1), 8L);
        newMieter.setStrasse("Strasse");
        newMieter.setPlz("3000");
        newMieter.setOrt("Bern");
        newMieter.setMietende(LocalDate.of(2025, 12, 31));
        newMieter.setLadepunkt("   ");

        when(organizationContextService.getCurrentOrgId()).thenReturn(testOrgId);
        when(mieterRepository.existsOverlappingMieterBounded(eq(8L), any(), any(), eq(-1L)))
                .thenReturn(false);
        when(mieterRepository.save(any(Mieter.class))).thenAnswer(invocation -> invocation.getArgument(0));
    ...
    @Test
    void saveMieter_ExistingMieterKeepsOwnLadepunkt_ExcludesItself() {
        ...
        assertEquals("LP-01", result.getLadepunkt());
        // Die eigene ID muss von der Eindeutigkeitspruefung ausgenommen sein
        verify(mieterRepository).existsByLadepunkt("LP-01", 1L);
    }
