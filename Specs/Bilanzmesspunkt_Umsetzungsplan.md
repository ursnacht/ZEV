...
| [x] | 11. MQTT-Splitting (Nachtrag) | Gemeinsamer Bilanzmesspunkt: `EinheitRepository.findAllByOrgIdAndMesspunkt` (mehrere Treffer zulässig), `MqttIngestService` splittet die Meldung je Einheit mit Register-Projektion (BEZUG: nur Bezug, RUECKLIEFERUNG: nur Einspeisung, jeweils andere = 0; PRODUCER/CONSUMER verbatim). `ZaehlerAggregationService` unverändert korrekt (arbeitet je Einheit auf projizierten Rohdaten). Simulator: neuer Modus `bilanz` (beide Register in einer Meldung). |
| [x] | 12. Bilanz-Zeilen ersetzen berechnete Zeilen (Nachtrag) | FR-5.7/FR-4.6 revidiert: Die berechneten Zeilen „Bezug von VNB"/„Rücklieferung" wurden aus der Werte-Tabelle **entfernt** (Web + PDF); die Berechnung bleibt und wird nur noch im Summen-Vergleich gegen die Bilanz-Einheiten verwendet. **Stattdessen** erscheinen die Bilanz-Summen als konditionale Zeilen (Beschriftung = Einheiten-Name, Balken H/I); ohne Bilanz-Einheit werden Zeile **und** zugehöriger Vergleich ausgeblendet. Backend liefert `bilanzBezugName`/`bilanzRuecklieferungName` (`EinheitRepository.findFirstByTyp`, null = keine Einheit). Bar-Skala (Web `getBarWidth`, PDF `maxSumme`) um Bilanz-Summen erweitert. |
...
- **Tooltips Bilanz-Vergleiche:** Beide Vergleichs-Items erläutern die Berechnung per nativem `title`-Tooltip (Keys `TOOLTIP_VERGLEICH_BEZUG`/`TOOLTIP_VERGLEICH_RUECKLIEFERUNG`, Migration `V81`); Muster wie in `datenbank-ansicht`/`lizenzen`, kein neues CSS.
- **Bezug-Vergleich auf gemessenen Wert (FR-4.1 revidiert):** „Bezug von VNB" rechnet neu `ConsumerTotal − zev Consumer` (B, **gemessen**) statt `− zev_berechnet` (C) – Messung gegen Messung beim Bilanz-Vergleich; bei MQTT-Daten identisch (B = C). „Rücklieferung" unverändert `ProducerTotal − zev Producer` (A). Seitenreine Zuordnung bewusst beibehalten (Fehlerlokalisierung; Kreuz-Konsistenz prüfen A=B/A=C/B=C).
