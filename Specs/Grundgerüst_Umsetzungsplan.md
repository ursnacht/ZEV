...
|--------|-------|--------------|
| [x] | 1. Root-Verzeichnis & Parent-POM | `/data/git/glue-starter/` anlegen; `pom.xml` (ch.glue, starter-parent, Java 25), `.gitignore`, `.env.example` |
| [x] | 2. Design System | `design-system/` kopieren; `package.json`: `@zev/design-system` → `@glue/design-system`; alle CSS-Klassen `zev-` → `app-`; `npm run build` |
| [x] | 3. Admin Service | `admin-service/` kopieren; Package `ch.nacht` → `ch.glue`; Parent-Referenz anpassen; `mvn compile -pl admin-service` |
| [x] | 4. Backend – Projekt-Setup | `backend-service/pom.xml` (ch.glue, starter-backend, Java 25, deps ohne JasperReports/Spring AI/QR-Bill, cyclonedx-plugin + log4j-fix); `BackendServiceApplication.java`; `application.properties`; `Dockerfile` |
| [x] | 5. Backend – Core-Infrastruktur | `SecurityConfig` (Rollen `user`/`admin`), `WebMvcConfig`, `CacheConfig` (nur `lizenzen`), `OrganizationInterceptor`, `OrganizationContextService`, `HibernateFilterService`, `OrganisationService`, `Organisation`-Entity, `OrganisationRepository`, `GlobalExceptionHandler`, `NoOrganizationException`, `PingController`, `package-info.java` |
| [x] | 6. Backend – Translation | `Translation`-Entity, `TranslationRepository`, `TranslationService`, `TranslationController` (Rolle `admin`); `mvn compile -pl backend-service` |
| [x] | 7. Backend – Lizenzen / SBOM | `LizenzenDTO`, `LizenzenHashDTO`, `LizenzenService`, `LizenzenController` (Rolle `user`); `CacheConfig` um `lizenzen` ergänzen; `mvn compile -pl backend-service` |
| [x] | 8. Backend – DB-Migrationen | `V1__Create_Schemas.sql` (Schema `app` + `keycloak`), `V2__Create_Translation_Table.sql`, `V3__Create_Organisation_Table.sql`, `V4__Add_Initial_Translations.sql` (alle Generic-Translations: Navigation, Dark Mode, Lizenzen, Translation-Editor, Startseite) |
| [x] | 9. Frontend – Projekt-Setup | `package.json` (`@glue/design-system`, Angular 21.2.6, ohne ZEV-spezifische Deps); `angular.json`; `tsconfig*.json`; `karma.conf.js`; `playwright.config.ts`; `Dockerfile`; `nginx.conf`; `scripts/generate-licenses.js`; `src/index.html` (Dark-Mode-FOUC-Script); `src/styles.css`; `src/main.ts` |
| [x] | 10. Frontend – Core | `app.config.ts`; `guards/auth.guard.ts`; `interceptors/error.interceptor.ts`; `pipes/translate.pipe.ts`; `pipes/swiss-date.pipe.ts`; `directives/column-resize.directive.ts`; `utils/date-utils.ts`; `services/translation.service.ts`; `services/theme.service.ts`; `services/lizenzen.service.ts`; `models/lizenzen.model.ts` |
| [x] | 11. Frontend – App & Navigation | `app.component.ts/.html/.css`; `app.routes.ts` (Routen: `startseite`, `translations`, `lizenzen`, `design-system`); `NavigationComponent` (nur generische Menüeinträge: Startseite, Übersetzungen, Lizenzen, Dark-Mode-Toggle, Sprachwahl, Logout) |
| [x] | 12. Frontend – Seiten-Komponenten | `IconComponent` + `icons.ts`; `KebabMenuComponent`; `StartseiteComponent` (generischer Willkommenstext + Logo-Platzhalter); `TranslationEditorComponent`; `LizenzenComponent`; `DesignSystemShowcaseComponent`; `ng build --configuration=development` |
| [x] | 13. Frontend – Tests | `tests/helpers.ts` (angepasst auf `user`/`admin`-Rollen und Realm `glue`); `tests/app.spec.ts` (Login, Navigation); `tests/lizenzen.spec.ts` (kopiert aus ZEV) |
| [x] | 14. Docker Compose & Keycloak Realm | `docker-compose.yml` (ohne Prometheus/Grafana; DB `app`, Realm `glue`, alle Service-Namen ohne `zev`-Präfix); `keycloak/realms/glue-realm.json` (Realm `glue`, Rollen `user`/`admin`, Client `starter-frontend`, Testuser `testuser`/`testpassword` → `admin`, `user`/`password` → `user`) |
| [x] | 15. CLAUDE.md | Vollständige Adaption: `ch.nacht` → `ch.glue`, `zev` → `starter`, `zev-` → `app-`, Java 21 → 25, Rollen, DB-Schema, keine ZEV-spezifischen Controller/Services, Code-Vorlagen auf generische Klassen anpassen |
| [x] | 16. Specs & Skills | `Specs/SPEC.md` (Rollen `user`/`admin`, Präfix `app-`); `Specs/generell.md` (Java 25, Angular 21, `app-`, MCP `glue-db`); alle `.claude/commands/*.md` anpassen (groupId, Präfix, Rollen, Pfade) |
| [x] | 17. MCP Server `glue-db` | `.claude/settings.local.json` mit MCP-Server-Konfiguration für `glue-db` (PostgreSQL, DB `app`) |
| [x] | 18. Validierung | `mvn compile` (Backend), `ng build` (Frontend), `npm run build` (Design System), `docker-compose up --build` (End-to-End-Smoke-Test) |
