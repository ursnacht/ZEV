...
Umgesetzt wird [`Abrechnung.md`](./Abrechnung.md): Nebenkostenabrechnungen je Zeitraum mit drei
Positionsarten (Umlage, Verbrauch, Zuschlag), Positionen und Akonto je Mieter, Saldo als
...
| `entity/NkPositionsart.java` | neu — `UMLAGE`, `VERBRAUCH`, `ZUSCHLAG` |
...
| `repository/EinheitRepository.java` | **neu:** `countByTyp` für die Vorbelegung |
...
|--------|-------|--------------|
| [ ] | 1. Mengeneinheit `M3` | Enum-Wert + Migration V117 für `ck_tarif_mengeneinheit`. Frontend-Enum und **beide** Key-Funktionen in `tarif.model.ts` mitziehen — sonst wird `M3` als „kWh" beschriftet. |
| [ ] | 2. Migration Tabellen | V118: `nk_abrechnung`, `nk_position`, `nk_verbrauch`, `nk_zusatz`, `nk_akonto` samt Sequenzen, CHECK- und Unique-Constraints, Spaltenkommentaren. Mieter-FKs auf **`ON DELETE RESTRICT`**. |
| [ ] | 3. Migration Akonto-Stammdatum | V119: `zev.mieter.akonto_pro_monat NUMERIC(10,2)` nullable. |
| [ ] | 4. Entities und Repositories | Fünf Entities nach dem Muster von `Tarifposition.java` (`@Filter(orgFilter)`, Sequenz-Generator), Repositories dazu. `Mieter` um das neue Feld erweitern. |
| [ ] | 5. Hilfsabfragen | `EinheitRepository.countByTyp`; `MieterRepository`-Abfrage für Mieter im Zeitraum. |
| [ ] | 6. **Berechnungsservice** | `NkBerechnungService`: Tage je Mieter, Umlage zeitanteilig, unverteilter Leerstandsanteil, Verbrauch, Zusatz, Zuschlagskaskade über beide Tabellen inkl. Gleichstandsregel, Akonto und Saldo. **`BigDecimal`, `setScale(2, HALF_UP)` je Zeile.** Ohne Persistenz, ohne Spring-Kontext — reine Funktionen auf Eingabedaten. |
| [ ] | 7. Service | `NkAbrechnungService`: CRUD, `enableOrgFilter()` **und** `pruefeFeatureFlag()` am Anfang jeder Methode, Schreibschutz bei `abgerechnet`, Validierung `Σ Tage(i) <= Nenner`. |
| [ ] | 8. DTOs und Controller | Zusammengesetzte Antwort für `GET /{id}` (alles in einem Aufruf, NFR-1); Endpunkte nach FR-6 inkl. `PATCH .../abgerechnet`. `@PreAuthorize` auf Klassenebene. |
| [ ] | 9. Löschschutz Mieter | `MieterService.deleteMieter` um die NK-Prüfung erweitern, damit statt des Datenbankfehlers eine verständliche Meldung erscheint. |
| [ ] | 10. ArchUnit-Regel | Test, der sicherstellt, dass jede öffentliche Methode des NK-Service den Feature-Flag prüft. |
| [ ] | 11. Frontend-Modell und -Service | `nebenkosten.model.ts`, `nebenkosten.service.ts` nach dem Muster von `tarif.service.ts`. |
| [ ] | 12. Liste | Gerüstseite ersetzen: `zev-table`, Sortierung nach `datum_von` absteigend, Inline-Checkbox „abgerechnet" mit Rückfrage nur beim Deaktivieren, Kebab-Menü, Schaltfläche **unterhalb** der Tabelle. |
| [ ] | 13a. `@angular/cdk` aufnehmen | `npm install @angular/cdk@^21` — Entscheid 1. Vor Phase 13, weil die Positionstabelle darauf aufbaut. |
| [ ] | 13. Maske: Kopf und allgemeine Positionen | Angaben zur Abrechnung inkl. **Anzahl Wohnungen** (vorbelegt, leer wenn keine `CONSUMER`-Einheiten), Positionstabelle mit art-abhängigen Feldern und **Drag & Drop**. |
| [ ] | 14. Maske: Mieterblöcke | Je Mieter `zev-panel` mit Umlage-, Verbrauchs-, Zusatz- und Zuschlagszeilen, Kostentotal, Akonto und Saldo; Hinweis bei Mietern ohne Einheit. |
| [ ] | 15. Sofortberechnung | `utils/nebenkosten-berechnung.ts` — dieselben Regeln clientseitig als **Vorschau**; Anzeige von Leerstandsanteil und Rundungsdifferenz getrennt. **Nach dem Speichern werden die Werte des Servers geladen und angezeigt** (Entscheid 2), nicht die selbst gerechneten. |
| [ ] | 16. Übersetzungen | V120 mit allen Schlüsseln aus FR-7, je deutsch und englisch. |
| [ ] | 17. Sperre | Alle Eingabefelder bei `abgerechnet` sperren, Hinweis anzeigen; nur das Flag bleibt bedienbar. |
