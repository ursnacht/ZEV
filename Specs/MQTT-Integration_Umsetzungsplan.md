...
| [x] | 14. Nachtrag (Vibe): Verteilung nach Aggregation (FR-6.7) | `ZaehlerAggregationService.aggregiere()` sammelt je `org_id` das min/max verarbeitete Intervallende und ruft nach der Schleife `MesswerteService.calculateSolarDistributionForOrg(org, von, bis, "PROPORTIONAL")` je Mandant (Fehler pro Mandant geloggt, kein Abbruch). **Neu in `MesswerteService`:** Loop in `distribute(from,to,algo,progressOrgId)` extrahiert; `calculateSolarDistribution` (Request-Pfad, unverändert) + `calculateSolarDistributionForOrg` (expliziter Mandant, kein Progress). **`HibernateFilterService.enableOrgFilter(Long orgId)`**-Overload (setzt/aktualisiert `orgFilter` explizit; no-arg delegiert). Tests: `aggregiere_RuftSolarverteilungFuerBehandeltenZeitraum`, `aggregiere_KeinMesswert_KeineSolarverteilung`. |
| [x] | 15. Nachtrag (Vibe): Monitoring Zähler-Ausfälle (FR-8.3) | `ZaehlerAggregationService.meldeDatenluecke(...)`: Lückenlänge = `Duration.between(referenz.zeit, intervallStart) / 15min`. **0** → keine Meldung (auch bei einzelnen ausgefallenen Messungen innerhalb eines Intervalls – absolute Stände gleichen sie aus), **1** → `INFO` `MQTT_ZAEHLER_LUECKE`, **≥2** → `WARN` `MQTT_ZAEHLER_AUSFALL`. Meldung via `SystemmeldungService.erfasse(einheit.orgId, …)` (org-explizit, dedupliziert je Key); Kategorie/Keys als Konstanten **im `SystemmeldungService`**. Neuer Konstruktor-Parameter `SystemmeldungService`. Übersetzungen `V93` (DE/EN). Tests: `aggregiere_LueckenlosOhneAusfall_KeineSystemmeldung`, `…_EinIntervallOhneDaten_ErfasstInfoMeldung`, `…_MehrereIntervalleOhneDaten_ErfasstWarnMeldung`. |
