...
6. **Autarkiegrad (gemessen)** = `1 − B / C` und **Netzbezugsquote (gemessen)** = `B / C`, mit dem **gemessenen** Netzbezug `B = bilanzBezug` der `BEZUG`-Bilanz-Einheit. Nur wenn eine `BEZUG`-Einheit existiert und `C > 0`; sonst entfallen beide Zeilen.
   * **Warum beide Werte nebeneinander:** Autarkiegrad (1) rechnet aus dem ZEV-Anteil der Konsumenten, also aus der **direkt** verbrauchten Produktion. Der gemessene Wert rechnet aus dem Zähler am Netzanschluss. Deckt eine **Batterie-Entladung** Verbrauch, ist dieser Anteil weder im `zev` der Konsumenten noch im Netzbezug enthalten – die beiden Kennzahlen fallen dann auseinander, und **genau diese Differenz ist der Beitrag der Batterie**. Beispiel: `C=985`, `Cz=888.5`, `B=38` → Autarkiegrad `90.2 %`, Autarkiegrad (gemessen) `96.1 %`; Differenz `58.5 kWh` aus der Batterie.
   * Die gemessenen Werte stehen in der Tabelle **direkt hinter** ihrem gerechneten Gegenstück, damit die Differenz beim Lesen auffällt.
   * **Lücken in der Bilanzmessung:** Fehlt der `BEZUG`-Messwert für einzelne Intervalle (FR-2.5 Bilanzmodell), ist `B` zu klein – der gemessene Autarkiegrad fällt zu hoch, die gemessene Netzbezugsquote zu tief aus. Die tages- und einheitengenaue Vollständigkeitsprüfung sieht solche Lücken **innerhalb** eines Tages nicht. Deshalb wird die Intervall-Abdeckung des `BEZUG` gegen die der Konsumenten gezählt; bei Unterdeckung werden beide Zeilen als **„lückenhaft"** gekennzeichnet (Wert wird trotzdem angezeigt).
...
* [ ] Deckt der `BEZUG` weniger 15-Min-Intervalle ab als die Konsumenten, tragen beide gemessenen Zeilen die Kennzeichnung **„lückenhaft"** mit erklärendem Hinweis; der Wert wird trotzdem angezeigt.
