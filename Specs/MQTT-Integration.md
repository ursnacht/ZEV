...
* [ ] **Zählertausch-Erkennung (Blindspot):** Der Reset-Guard erkennt einen Tausch nur am **negativen** Delta. Startet der neue Zähler **höher** als der alte endete, ist das Delta positiv → der Sprung wird als echter Verbrauch verbucht (Bogus-Wert, keine Warnung). Aus den Werten allein nicht zuverlässig erkennbar → **explizites Signal nötig.** Vorschlag: **Zähler-Seriennummer (`zaehlerId`) im Payload + `zaehler_rohdaten`**; die Aggregation setzt bei Serien-Wechsel eine neue Baseline (kein Delta über die Grenze, robust in beide Richtungen). Alternativ app-interner Wechsel-Marker je Einheit. (Interim: neuen Zähler auf Endstand voreinstellen oder Übergangsintervall manuell korrigieren.)
