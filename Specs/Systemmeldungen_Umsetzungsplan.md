...
| [x] | 15. Nachtrag (Vibe): Audit-Meldung „Solar-Verteilung manuell gestartet" | INFO-Meldung je **manuellem** Verteillauf (Spec FR-1.11). **Service:** neue Konstanten `KATEGORIE_VERTEILUNG` / `KEY_VERTEILUNG_MANUELL` + neue Methode `erfasseAudit(orgId, level, kategorie, meldungKey, parameter)` – `REQUIRES_NEW`, **ohne** Dedup (ein Eintrag pro Lauf) und **direkt erledigt** (`erledigt=true`, `erledigtAm=jetzt`, `erledigtAutomatisch=true`), weil der UNIQUE-Teil-Index nur einen **offenen** Eintrag je (`org_id`,`meldung_key`) zulässt; `parameter` wird auf 500 Zeichen gekürzt. **Controller:** `MesswerteController.calculateDistribution` erhält `@AuthenticationPrincipal Jwt` (Claim `preferred_username`, Muster aus `AuthController`) + `SystemmeldungService` im Konstruktor; private Hilfsmethode `erfasseManuellenStart(...)` schreibt **vor** dem Lauf `benutzer, dd.MM.yyyy–dd.MM.yyyy, algorithm` und schluckt Fehler (nur `log.warn`). Der MQTT-Auto-Lauf bleibt unberührt. Übersetzungen `V97` (DE/EN). |
