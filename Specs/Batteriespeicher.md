...
* **Aktueller Stand:**
  ...
  - Producer-`zev` = im ZEV konsumierte Produktion (= im Intervall verteilte Menge).
...
4. **`zev` der Speicher-Einheit** (nur `quelle = MQTT`, vorzeichenbehaftet wie `total`):
...
5. **Producer-`zev` berücksichtigt den Speicher:** Die im ZEV verwendete PV-Produktion umfasst neu auch die ZEV-Ladung: Producer-`zev` (Summe, quellen-proportional `× P/Q`) = `(D + ZEV-Ladung) × P/Q`; bei mehreren Producern wie bisher proportional zur Produktion aufgeteilt. CSV-Producer bleiben unangetastet (gemessene Werte).
6. Consumer-`zev`/`zev_calculated` bleiben wie bisher = zugeteilte Menge (Quelle PV oder Speicher ist für den Consumer transparent, gleicher ZEV-Tarif).

### FR-4: Statistik & Bilanz-Vergleiche
1. Die Summen A–E (Producer/Consumer) enthalten **keine** Speicher-Werte.
2. **Neue Werte-Zeilen** (nur falls Speicher-Einheit existiert, Beschriftung = Einheiten-Name mit Zusatz): „`<Name>` (Ladung)" = Summe der positiven `total`-Intervalle, „`<Name>` (Entladung)" = Betrag der Summe der negativen Intervalle; mit Balken (gleiche Skala), Web + PDF. Keys `STATISTIK_LADUNG`/`STATISTIK_ENTLADUNG` für den Zusatz.
3. **Angepasste Vergleichswerte** (Summen-Vergleich gegen die Bilanz-Einheiten):
   - `Rücklieferung (berechnet) = |ProducerTotal| + Entladung − zev(Producer) − |zev(Speicher)|` — Produktion und Entladung, die weder von Consumern konsumiert noch in den Speicher geladen wurden.
   - `Bezug von VNB (berechnet) = ConsumerTotal − zev(Consumer) + Netzladung` mit `Netzladung = Ladung − ZEV-Ladung` (= `Ladung − zev(Speicher)` über die Ladungs-Intervalle).
   - Toleranz unverändert 0.1 kWh; ohne Speicher-Einheit sind die Formeln identisch zu heute.
4. **„Summen pro Einheit":** Der Speicher wird mitgelistet (Absolutwert von `total` = Saldo, `zev` = Absolutwert; „ZEV berechnet" = `-`). Sortierung: Produzenten, Konsumenten, **Speicher**, Rücklieferung, Bezug.
...
* Neue Übersetzungs-Keys via Flyway-Migration (`ON CONFLICT (key) DO NOTHING`): `TYP_SPEICHER` („Speicher"/„Storage"), `EINHEIT_SPEICHER_EXISTIERT` („Es existiert bereits eine Speicher-Einheit."/„A storage unit already exists."), `STATISTIK_LADUNG` („Ladung"/„Charge"), `STATISTIK_ENTLADUNG` („Entladung"/„Discharge").
...
* [ ] Producer-`zev` enthält die ZEV-Ladung (`P=10, V=6, L=5` → Producer-`zev` = 10); CSV-Producer bleiben unverändert.
* [ ] Ohne Speicher-Einheit ist das Verteilergebnis **identisch** zu heute (Regression).

### Statistik
* [ ] Existiert eine Speicher-Einheit, zeigt die Werte-Tabelle die Zeilen „`<Name>` (Ladung)" und „`<Name>` (Entladung)" mit Balken (Web + PDF); ohne Speicher entfallen sie.
* [ ] Die Vergleichswerte rechnen gemäss FR-4.3 (inkl. Netzladung); die Bilanz-Vergleiche gehen bei konsistenten Daten innerhalb der Toleranz auf.
...
| Entladung ohne Consumer im Intervall | nichts verteilt (`D = 0`), Speicher-`zev = 0`; Entladung erscheint als Rücklieferung |
...
* **Betroffener Code (Backend):**
  ...
  - `service/MesswerteService.java` — **Kern**: Speicher-Messwert je Zeitpunkt laden; Entladung in den Produktionspool, zweistufige Zuteilung (Consumer → ZEV-Ladung), `zev` für Speicher und Producer gemäss FR-3.4/3.5.
...
* [ ] **Quellen-Proportionalität** (FR-3.4/3.5, Faktor `P/Q` bzw. `E/Q`): exakte Zuordnung oder vereinfachend „Entladung zuerst verteilen"? (Ergebnis für Rechnungen identisch, betrifft nur die zev-Aufteilung Producer/Speicher in der Statistik.)
* [ ] **Statistik-Darstellung:** zwei Zeilen (Ladung/Entladung) wie spezifiziert oder zusätzlich eine Saldo-Zeile?
* [ ] Soll die Netzladung als eigener Statistik-Wert sichtbar sein (z.B. Tooltip/Zeile) oder nur in die Vergleichsformel einfliessen?
