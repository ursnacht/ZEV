...
| [x] | 14. Nachtrag (Vibe): Lese-Timeout konfigurierbar (FR-1.5) | Timeout war effektiv **fest 3 s**: `ModbusReader.__init__` hatte zwar einen `timeout`-Parameter, die Factory übergab ihn aber nie. Neu: globaler `read_timeout` (Default **5 s**, angehoben) in `GatewayConfig.read_timeout_seconds`, je Zähler via `zaehler[].read_timeout` überschreibbar (bewusst **derselbe** Schlüsselname wie global) → aufgelöst in `MeterConfig.read_timeout_seconds`. Neuer Parser `_parse_timeout` (Zahl oder `"8s"`/`"1m"`; `0`/negativ/nicht-numerisch/Boolean → `ConfigError` beim Start). `ModbusReader` liest `config.read_timeout_seconds` — **Konstruktor-Parameter entfernt**, damit es nur eine Quelle gibt; Factory-Signatur (`create_reader(config)`) bleibt unverändert, also auch `SimReader`/`GplugReader` unberührt. Fehlermeldung unterscheidet **ausbleibende Antwort** (nennt Timeout + Hinweis) von **Modbus-Exception-Response** (dort hilft das Timeout nicht). Beispiel-Configs + README ergänzt. |

| [x] | 15. Nachtrag (Vibe): globales `read_timeout` beim Start ausgeben | Die Start-Log-Zeile in `gateway/main.py` nennt zusätzlich `Lese-Timeout=%.1fs` (`config.read_timeout_seconds`) – damit ist der wirksame Wert im `journalctl` sichtbar, auch wenn er aus dem Default stammt. Ergänzend neue Hilfsfunktion `_log_abweichende_timeouts(config)`: nennt Zähler, deren `read_timeout_seconds` vom globalen Wert abweicht (`messpunkt=Wert`), sonst keine Zeile — die Reader loggen ihren Wert nur auf `DEBUG`, ohne diesen Hinweis liesse sich der globale Wert als universell missverstehen. Reine Logging-Ergänzung: keine Änderung an Config-Schema, Reader-Verhalten oder Payload. |
| [x] | 16. Nachtrag (Vibe): Lese-Retry (FR-1.6) | Sporadische `exception_code=11` treffen **wechselnde** Zähler; am eingesetzten Modul **WAGO 879-9000** ist **kein RTU-Response-Timeout einstellbar** (nur Baudrate/Parität der RS485-Seite) → Kompensation client-seitig. Neu `read_attempts` (global, **inkl.** Erstversuch, Default **2**, `1` = kein Retry) in `GatewayConfig` + Parser `_parse_attempts` (ganze Zahl ≥ 1; `0`/negativ/float/Boolean/String → `ConfigError` beim Start). `main._read_with_retry(reader, attempts)` wiederholt nur `ReadError` (0,5 s Pause, Verbindung wird je Versuch neu aufgebaut, da `Reader.read()` sie schliesst); Zwischenfehlschläge auf `INFO` mit Versuchszähler, endgültiger auf `WARN`. **Kein** Retry bei unerwarteten Exceptions. Bewusst **reader-agnostisch** in `main` statt im `ModbusReader`, damit künftige Reader es erben. Unschädlich, weil absolute Stände publiziert werden. Start-Log nennt die Versuchszahl; Doku: `README.md`, `config.example.yaml`, `docs/Netzwerk-Topologie-Hene.md` (inkl. Korrektur der Aussage „Response-Timeout im Hub" + 879-9000-Fakten). |
| [x] | 17. Nachtrag (Vibe): `register.einspeisung` optional (FR-1.7) | Konsumenten speisen nicht ein → Block weglassbar. `MeterConfig.register_einspeisung` ist jetzt `RegisterSpec \| None`; neuer Parser `_parse_register_optional` gibt `None` zurück, wenn der Block **fehlt**, delegiert sonst an `_parse_register` (ein **vorhandener, fehlerhafter** Block bleibt `ConfigError` – ein Tippfehler darf nicht stillschweigend zu 0 werden). `ModbusReader.read()` liest das Register nur bei vorhandener Config, sonst `0.0` → **ein** Modbus-Zugriff statt zwei, entlastet die RS485-Strecke. `register.bezug` bleibt Pflicht. Beim Start je betroffenem Zähler eine `INFO`-Zeile, damit ein versehentlich fehlender Block auffällt. **Publisher unverändert:** `zaehlerstandEinspeisung` bleibt Pflichtfeld im Payload (mit 0) → Backend/Vertrag unberührt. `SimReader` unberührt (nutzt keine echten Register). Doku: `README.md`, `config.example.yaml` (Beispiel-Konsument ohne Block). |
